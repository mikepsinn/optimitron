...
- Use one reusable local web server by default. For `apps/optimitron`, prefer `http://127.0.0.1:3001` / `http://localhost:3001` as the canonical dev server. Before starting any new web server, check whether port `3001` is already serving the app and reuse it for browser checks, Playwright, screenshots, and ad-hoc verification.
...
- Generate the current screenshot review at `apps/optimitron/output/playwright/review/latest.html` by default, organized by page/viewport with before/after screenshots side by side when both versions are available. This gives the human one stable local file to bookmark and refresh after each UI change.
...
- If you create a named review folder, also update `apps/optimitron/output/playwright/review/latest.html`. Copy referenced screenshot assets beside `latest.html` or rewrite image paths relative to that stable file, then verify the stable page has no broken image references.
- Do not commit screenshot image artifacts to the repo unless the human explicitly asks and the screenshots are confirmed sanitized. Keep local artifacts under `apps/optimitron/output/playwright/` while working.
...
- If a change creates or alters generated user-facing artifacts such as tasks, emails, notifications, share templates, receipts, auth callback states, or post-submit success states, include a concrete preview of those artifacts in the review. Prefer a local page link, seeded example, or screenshot in `apps/optimitron/output/playwright/review/latest.html`; if no preview surface exists, say that and consider adding a safe preview route or fixture before calling the work done.
