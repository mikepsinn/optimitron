...
- **Evidence:** apps/optimitron/src/lib/mcp-server.ts; apps/optimitron/src/lib/tasks.server.ts (`createTask`); apps/optimitron/src/lib/tasks/task-comments.server.ts; tests: apps/optimitron/src/lib/**tests**/mcp-server.test.ts, **tests**/tasks.server.test.ts, **tests**/task-comments.server.test.ts
...
- **Evidence:** packages/db/prisma/schema.prisma (`model TaskEdge`, `enum TaskEdgeType`); `getBlockers` tool in apps/optimitron/src/lib/mcp-server.ts; `isTaskBlocked` in apps/optimitron/src/lib/tasks/rank-tasks.ts; apps/optimitron/src/lib/tasks/marginal-impact.ts + test
...
- **Evidence:** apps/optimitron/src/lib/triggers/admin.ts; apps/optimitron/src/lib/triggers/fire.ts; tests: triggers/**tests**/fire.integration.test.ts, triggers/**tests**/one-percent-treaty-blueprints.test.ts
...
- **Evidence:** apps/optimitron/src/lib/tasks/execution-planner-audit.ts; apps/optimitron/src/lib/tasks/task-tree-steward.ts; apps/optimitron/src/lib/tasks/task-tree-steward.server.ts; execution-planner-audit.test.ts; task-tree-steward.test.ts; wired into `getQueueAudit` and `getTaskTreeAudit` in apps/optimitron/src/lib/mcp-server.ts
...
- **Evidence:** `createTask`, `proposeTaskBundle`, `addDependency` tools in apps/optimitron/src/lib/mcp-server.ts
...
- **Evidence:** `validateExplicitTaskParent` in apps/optimitron/src/lib/mcp-server.ts (throws on root); apps/optimitron/src/lib/tasks.server.ts `createTask` / `resolveDefaultPrivateParent` (private parentless → `ensureExecutionPlanningBranch`); apps/optimitron/src/lib/feedback.server.ts (`parentTaskId: OPTIMITRON_DEV_TASK_ID`)
...
- **Evidence:** `proposeTaskBundle` in apps/optimitron/src/lib/mcp-server.ts; `EXECUTABLE_PARENT` finding in apps/optimitron/src/lib/tasks/execution-planner-audit.ts + test
...
- **Evidence:** `docs/plans/phased-approach-optimitron.md`; `packages/db/prisma/migrations/20260715180000_private_execution_system/`; `apps/optimitron/src/lib/mcp-tools/private-execution.ts`; `apps/optimitron/src/lib/tasks/private-task-bundle.server.ts`; `apps/optimitron/src/lib/tasks/execution-lifecycle.server.ts`; `apps/optimitron/src/lib/tasks/external-action.server.ts`; `apps/optimitron/src/lib/tasks/private-work-portability.server.ts`; `apps/optimitron/src/lib/tasks/task-visibility.server.ts`; `apps/optimitron/src/lib/source-artifact-visibility.server.ts`
...
- **Evidence:** apps/optimitron/src/lib/tasks/rank-tasks.ts (`computeTaskPriority`, `assessTaskForUserCapability`); packages/agent/src/task-capability.ts; migration packages/db/prisma/migrations/20260711190000_remove_task_difficulty/; rank-tasks.test.ts
...
- **Evidence:** apps/optimitron/src/lib/tasks/execution-planner.ts (`buildExecutionPlan`) wired to the `getExecutionPlan` MCP tool in apps/optimitron/src/lib/mcp-server.ts; execution-planner.test.ts; execution-planner-pilot.test.ts
...
- **Evidence:** packages/db/prisma/schema.prisma (`TaskImpactEstimateSet`, `TaskImpactFrameEstimate`); apps/optimitron/src/lib/tasks/impact.ts (`deriveImpactRatios`, `getNormalizedImpactComponents`); impact.test.ts
...
- **Evidence:** absence verified in `getMyQueue` / `getAIQueue` / `getNextAction` handlers in apps/optimitron/src/lib/mcp-server.ts (no measurement/health/medication logic)
...
- **Evidence:** apps/optimitron/src/lib/profile.server.ts (`upsertDailyMeasurement`, `saveDailyCheckIn`); apps/optimitron/src/app/api/profile/check-in/route.ts; apps/optimitron/src/app/check-in/page.tsx
...
- **Evidence:** packages/chat-ui/src/nlp/text-to-measurements.ts; apps/optimitron/src/components/chat/ChatPage.tsx (parse without persistence — no call to upsertDailyMeasurement or any Measurement API)
...
- **Evidence:** `recordInterventionExperience` in apps/optimitron/src/lib/earth-data.server.ts, exposed in apps/optimitron/src/lib/mcp-server.ts
...
- **Evidence:** absence verified in apps/optimitron/src/lib/triggers/ and app/api/cron/run-due-triggers/route.ts (no cron.\* trigger spawning measurement tasks)
...
- **Evidence:** packages/optimizer/src/change-from-baseline.ts + test; sole consumer packages/extension/src/workers/analysis.worker.ts; zero references in apps/optimitron/src
...
- **Evidence:** packages/optimizer/src/ (pipeline.ts, statistics.ts, response-curve.ts, change-from-baseline.ts, …); consumed by apps/optimitron/src/app/api/health-analysis/submit/route.ts and lib/aggregate-relationships.server.ts
...
- **Evidence:** packages/wishocracy/src/; apps/optimitron/src/lib/wishocracy-bridge.ts; apps/optimitron/src/app/agencies/dcongress/wishocracy/page.tsx
...
- **Evidence:** schema.prisma (`ReferralInvitation`, `ShareAttempt`); apps/optimitron/src/components/landing/TreatyPostVoteShareFlow.tsx; components/dashboard/ReferralInvitationStatusCard.tsx
...
- **Evidence:** `signReferendumAsOrganization` in apps/optimitron/src/lib/earth-data.server.ts and mcp-server.ts
...
- **Evidence:** `claimSignerReminder` in apps/optimitron/src/lib/mcp-server.ts; apps/optimitron/src/lib/tasks/overdue-signers.server.ts + test; treaty-signer-network.ts
...
- **Evidence:** packages/treasury-prize/contracts/; packages/treasury-shared/src/addresses.ts (zero addresses); apps/optimitron/src/hooks/useTreasuryData.ts (`isDeployed` check + DEMO_DATA fallback)
...
- **Evidence:** packages/treasury-wish/contracts/; packages/treasury-shared/src/addresses.ts; apps/optimitron/src/components/treasury/DistributeUBICard.tsx (demo fallback)
...
- **Evidence:** apps/optimitron/src/lib/tasks/agent-lease.server.ts; `getAIQueue`, `reviewTaskApplication` in apps/optimitron/src/lib/mcp-server.ts; absence of a propose/approve/execute gate verified
...
- **Evidence:** apps/optimitron/src/lib/notion-import.schema.ts; apps/optimitron/src/lib/notion-import.server.ts; apps/optimitron/scripts/import-notion-bundle.ts; apps/optimitron/src/lib/**tests**/fixtures/notion-operational-workspaces.ts; @notionhq/client absent from every package.json
...
- **Evidence:** packages/db/prisma/schema.prisma (`Document`, `DocumentRevision`, `Collection*`, `ContentAccessGrant`, `ContentAttachment`); apps/optimitron/src/lib/documents.server.ts; apps/optimitron/src/lib/collections.server.ts; apps/optimitron/src/components/collections/collection-records-grid.tsx; apps/optimitron/src/lib/content-access.server.ts; apps/optimitron/src/lib/content-search.server.ts; REST/OpenAPI/MCP tests
...
- **Evidence:** packages/db/prisma/schema.prisma (`Form*`, `KnowledgeAnswer`); apps/optimitron/src/lib/form-responses.server.ts; apps/optimitron/src/lib/mcp-tools/form-responses.ts; apps/optimitron/src/lib/form-responses.server.test.ts; Task, DocumentRevision, TaskExecutionArtifact, TaskVerification, and ExternalActionRequest models
...
- **Evidence:** `normalizeCalendarForPlanning` in apps/optimitron/src/lib/tasks/execution-source-normalization.ts + tests; googleapis absent from every package.json
...
- **Evidence:** apps/optimitron/src/app/openapi.json/route.ts (+ test); apps/optimitron/src/app/developers/page.tsx; apps/optimitron/src/lib/mcp-oauth.ts; apps/optimitron/src/lib/mcp-scopes.ts
...
- **Evidence:** apps/optimitron/src/components/tasks/blocks/ (12 files); zero real imports verified repo-wide (only doc-string mentions in mcp-server.ts / task-context.ts)
