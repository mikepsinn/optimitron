...
pnpm --filter @optimitron/obg test

# Generate US optimal budget v6 report (minimum effective spending + efficient frontier)
pnpm --filter @optimitron/examples exec tsx src/us-federal-analysis/generate-efficient-frontier-report.ts
...
pnpm --filter @optimitron/obg test -- src/__tests__/policy-hypotheses.test.ts

# Run country analysis hypothesis tests
pnpm --filter @optimitron/obg test -- src/__tests__/country-analysis-hypotheses.test.ts

# Run efficient frontier hypothesis tests (Japan/Korea efficient, US inefficient)
pnpm --filter @optimitron/obg test -- src/__tests__/efficient-frontier-hypotheses.test.ts
```

### US Federal Budget Analysis (Full)
```bash
# Generate comprehensive budget analysis with BIS scores
pnpm --filter @optimitron/examples exec tsx src/us-federal-analysis/generate-budget-analysis.ts
...
pnpm --filter @optimitron/web run build
pnpm --filter @optimitron/web run start
...
pnpm --filter @optimitron/examples run generate:mega-studies

# Run the agent-assisted publication review on mega-study-api.json
pnpm --filter @optimitron/examples run review:mega-studies

# Or do both in one step
pnpm --filter @optimitron/examples run generate:mega-studies:reviewed
```
Review artifact:
...
| Package | Purpose |
| --- | --- |
| `@optimitron/optimizer` | Core causal inference engine |
| `@optimitron/obg` | Optimal Budget Generator |
| `@optimitron/data` | Datasets and importers |
| `@optimitron/db` | Prisma schema and types |
| `@optimitron/web` | Next.js website |
| `@optimitron/examples` | Analysis scripts and demos |
