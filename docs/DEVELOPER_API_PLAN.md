...
- Publish `/openapi.json` for the REST and OAuth surface that already exists.
- Add `/developers` as the human-readable entry point for external app and
  website builders.
- Reuse the existing OAuth authorization-code plus PKCE flow, dynamic client
  registration, refresh token rotation, and revocation endpoints.
...
- Keep MCP and REST on the same scope catalog.
