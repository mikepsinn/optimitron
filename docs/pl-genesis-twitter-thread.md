# PL Genesis Community Vote X Thread
...
**5/**
We integrated **Storacha** so aggregation snapshots and policy analyses are stored as content-addressed history chains. In other words, if the analysis changes later, it now leaves a trail, which is rude but fair.

**6/**
We integrated **Hypercerts** so every policy recommendation becomes a verifiable activity claim with measurements for PIS, CCS, welfare, evidence grade, citizen preference weight, government allocation, and preference gap. Bureaucracy, but with receipts.

**7/**
We integrated **ERC-8004** so the autonomous policy analyst has a machine-readable identity and reputation layer. Because if software is going to publish governance advice, it should at least have the decency to sign its homework.

**8/**
We also added a Gemini reasoning layer so the agent can discover tractable gaps, interpret raw causal outputs, decide whether evidence is strong enough to publish, and log the whole chain in structured JSON instead of inspirational fog.

**9/**
This lines up with the PL focus areas we care about most:
- **Crypto**: Hypercerts + ERC-8004 receipts
- **AI**: autonomous Gemini-guided analysis and verification
