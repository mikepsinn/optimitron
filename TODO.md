...
- **Unify seed.ts + managed-data sync?** (discussed 2026-05-11) Decision: NOT now. Both have legitimate but different roles — sync is idempotent canonical app data that runs in prod; seed.ts is dev/CI bootstrap convenience. Long-term, candidates to migrate from seed.ts into managed-data as we touch them: `task-triggers`, `government-leaders`, `conflicts`, `wishocratic-items`, possibly `drug-approval-timelines`. Reference data (units, variable categories, medical) probably stays in seed.ts — too high-volume and too stable to re-hash on every deploy.
