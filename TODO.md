...
- **Unify seed.ts + managed-data sync?** (discussed 2026-05-11) Decision: YES, incrementally. My earlier "high-volume data is too expensive to hash" argument was wrong — SHA-256 over the entire seed.ts content is sub-millisecond, and managed-data already skips DB writes on unchanged rows, so the per-deploy cost when nothing changed is essentially zero (read + hash + compare, no roundtrip). Grandma Kay + demo user are product-facing narrative fixtures (canonical "represented vote on behalf of grandmother" demo + tour onboarding) and SHOULD be in prod, not gated behind `seed:bootstrap`. Migration order, smallest blast radius first:
  1. **grandma-kay-example** → `packages/db/src/managed-data/managed-grandma-kay.ts`. Single person + a couple measurements + one represented-treaty-vote. ~60 lines.
  2. **demo-user** → `packages/db/src/managed-data/managed-demo-user.ts`. Single User + Person. ~30 lines.
  3. **task-triggers** → `packages/db/src/managed-data/managed-task-triggers.ts`. Replaces `db:seed:triggers` (currently runs separately on deploy per the P0 section above).
  4. **government-leaders / conflicts / wishocratic-items** → managed-data; small fixed catalogs.
  5. **drug-approval-timelines** → managed-data; sourced from `@optimitron/data/datasets/medical`.
  6. **reference data** (units, variable categories, global variables, medical reference data, jurisdictions) → managed-data last; biggest volume but hashing is still cheap. The blocker is that variable categories and global variables have foreign-key dependencies on units, so the sync needs to honor declaration order.
  When the migration is complete, `seed.ts` becomes a thin compatibility shim that just calls `db:sync:managed-data --apply --scope all`, and the `seed:bootstrap` / `seed:demo` / `seed:tasks` package scripts become deprecated aliases.
