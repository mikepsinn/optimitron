...
**Subagents** live in `.claude/agents/`: `voice-critic` (copy critique), `cold-stranger-ux` (zero-context UX reaction), `visual-design-auditor` (visual slop), `pr-comment-triager` (bot-review triage), `test-auditor` (suite slop + missing coverage), `security-threat-review` (OWASP+STRIDE), `root-cause-investigator` (debug discipline), `campaign-office-hours` (pre-code product interrogation), `campaign-scope-review` (Expansion/Selective/Hold/Reduction scope challenge). Skills in `.claude/skills/`: `qa` (one-command parallel critic pass — fires voice + cold-stranger + visual + tests + optional security), `verify-slide` (game slide screenshot check). Each `.md` file has the full instructions.
