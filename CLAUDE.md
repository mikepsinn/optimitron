...
- **Use local before/after review artifacts.** For meaningful visual changes, capture before and after screenshots when feasible and generate a side-by-side HTML review page under `packages/web/output/playwright/`. Link that file in the handoff with a clickable local file link and a plain filesystem path.
...
- **Do not commit or upload screenshot artifacts by default.** Keep images and HTML review pages local unless the user explicitly asks and the screenshots are confirmed sanitized. In PRs, prefer preview URLs, local artifact paths, and your visual-inspection notes over uploaded images.
