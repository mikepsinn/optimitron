...
- **Use local before/after review artifacts.** For meaningful visual changes, capture before and after screenshots when feasible and generate a full-width HTML review page under `packages/web/output/playwright/`, with the previous screenshot above the updated screenshot for each page/state/viewport. Link that file in the handoff with a clickable local file link and a plain filesystem path.
