...
**Reuse the dev server. Do not spawn a duplicate.** Before any `pnpm dev` / `dev:fast` / Playwright run that needs the dev server: `curl -sS -m 3 http://127.0.0.1:3001 -o /dev/null -w "%{http_code}\n"`. If it returns `200` (or any 2xx/3xx), REUSE IT — do not start a second server. If it returns nothing or times out but `netstat -ano | findstr :3001` (Windows) / `lsof -ti :3001` (macOS/Linux) shows the port is bound, a zombie is squatting — kill the PID (Stop-Process / kill) before starting fresh. Two `pnpm dev` instances on the same port causes EADDRINUSE; the second silently fails to bind and Playwright tests time out waiting for it. Applies to Claude, Codex, gstack subagents, and project-local subagents that drive Playwright.
