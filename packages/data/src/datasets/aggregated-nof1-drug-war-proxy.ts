...
export interface AggregatedNOf1DrugWarProxyRow {
  iso3: string;
  year: number;
...
export const DEFAULT_AGGREGATED_NOF1_DRUG_WAR_PROXY_PANEL_PATH = resolve(
  __dirname,
  '..',
  '..',
  'raw',
...
export function parseAggregatedNOf1DrugWarProxyCsv(
  csvText: string,
): AggregatedNOf1DrugWarProxyRow[] {
  const lines = csvText
    .split(/\r?\n/)
    .map((line) => line.trim())
  ...
  for (let rowIndex = 1; rowIndex < lines.length; rowIndex += 1) {
    ...
    rows.push({
      ...
      spendingCurrency: spendingCurrencyRaw && spendingCurrencyRaw.length > 0
        ? spendingCurrencyRaw
        : null,
    });
  }
...
export function loadAggregatedNOf1DrugWarProxyPanel(
  filePath: string = DEFAULT_AGGREGATED_NOF1_DRUG_WAR_PROXY_PANEL_PATH,
): AggregatedNOf1DrugWarProxyRow[] {
  const csvText = readFileSync(filePath, 'utf8');
  return parseAggregatedNOf1DrugWarProxyCsv(csvText);
}
