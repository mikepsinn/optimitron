...
export const OPTIMIZE_EARTH_PROTOCOL_STEPS = [
  ...
  'If the queue is clearly missing canonical campaign tasks, run pnpm db:sync:managed-data -- --apply before trusting the frontier.',
...
export function buildOptimizeEarthInstruction(input: OptimizeEarthPromptOptions = {}) {
  ...
  return [
    ...
    'If the queue is clearly missing canonical campaign tasks, run pnpm db:sync:managed-data -- --apply before calling getNextAction.',
