...
APPLICATIONS: work from one private application task owned by the applicant person or organization. Read the exact form, then call findReusableAnswers for each question. Call prepareApplicationQuestions with approved revision IDs where available; it reuses exact matches and creates only unresolved atomic verification tasks. Drafts are not approved answers. For each unresolved task, create or use its answer document, run startTaskExecution → submitTaskArtifact → submitTaskForVerification, and wait for explicit human acceptance through verifyTaskExecution. Call proposeApplicationSubmission only after every answer is an accepted immutable revision; it blocks placeholders and returns one exact pending payload. Never submit, publish, spend, or send from that proposal until the human approves the ExternalActionRequest. After execution, record the receipt with recordExternalActionResult. Do not invent facts, silently rewrite an approved answer, or create a second answer when an authorized current revision already exists.
