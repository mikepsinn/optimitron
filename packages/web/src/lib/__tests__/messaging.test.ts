...
describe("getUserFramingVocabulary", () => {
  it("returns the manager-frame block when asked", () => {
    ...
    expect(vocab.shareLinkPrompt).toContain("hiring link");
    expect(vocab.shareLinkPrompt).toContain("hire humanity managers");
  ...
  it("returns the voter-frame block when asked", () => {
    ...
    expect(vocab.shareLinkPrompt).toContain("referral code");
    expect(vocab.shareLinkPrompt).toContain("recruit voters");
    expect(vocab.org.longName).toBe("Earth Optimization Services LLC");
  });
