...
describe("treaty sender email sequence", () => {
  ...
  it("builds B3 and B4 sender nudges", () => {
    ...
    expect(second.text).toContain("You can't make them. But you can send to one more person and improve your odds.");
  });
  ...
  it("builds C1 never-shared re-engagement copy", () => {
    ...
    expect(email.text).toContain("[BUTTON: Tell one person → https://warondisease.org/send]");
  });
});
