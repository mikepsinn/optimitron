...
/**
 * Eight featured public systems from the 22-agency registry: the public
 * good each system optimizes, its future-state promise, one savings stat,
 * supporting replacement code, and the Earth agency's report-card grade.
 * All content is data-driven from the registries.
 */
...
export const FEATURED_PUBLIC_SYSTEMS: PublicSystemConfig[] = [
  {
...
function publicSystemHref(agencyId: string): string {
  return CANONICAL_AGENCY_HREFS[agencyId] ?? `${ROUTES.agencies}/${agencyId}`;
}

export function OptimizedPublicAdministration() {
  return (
    <div>
      <div className="er-public-system-grid">
        {FEATURED_PUBLIC_SYSTEMS.map(
          ({ agency, codeAnchor, extraLines, statValue, statCaption }) => {
            const grade = earthGradeFor(agency);
            const primaryOutcome = agency.optimalMetrics[0];
            return (
              <div className="er-public-system" key={agency.id}>
                ...
                <p className="er-public-system-purpose">{agency.tagline}</p>
                {primaryOutcome ? (
                <p className="er-public-system-outcome">
                ...
                <p className="er-public-system-stat">
                  {statValue ?? agency.annualSavings}
                </p>
                {statCaption ? (
                  <p className="er-caption">{statCaption}</p>
                ) : null}
                <code className="er-code-chip">
                  {excerptCode(agency.replacementCode, codeAnchor, extraLines)}
                </code>
                <Link
                  className="er-link er-mono mt-auto text-xs"
                  href={publicSystemHref(agency.id)}
                >
                  Explore this system
                </Link>
              </div>
      ...
      <p className="er-caption mt-4">
        The grade chip is the Earth agency&apos;s current report-card grade,
        computed from its spending trend against its outcome trend.
      </p>
      <div className="mt-8 text-center">
        <Link className="er-btn" href={ROUTES.agencies}>
          Explore all {WISHONIA_AGENCIES.length} agencies of the future
        </Link>
      </div>
    </div>
  );
