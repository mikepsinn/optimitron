...
const METHODS: GivingMethod[] = [
  {
    ...
    body: (
      <>
        <p>
          Recommend a grant from Fidelity Charitable, Schwab Charitable,
          Vanguard Charitable, or your DAF sponsor.
        </p>
        ...
        {NONPROFIT.dafDirectOrgId ? (
          <p className="text-xs text-neutral-600">
            Or use the DAF Direct widget below for one-click recommendation.
          </p>
        ) : null}
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          Direct your IRA custodian to send a distribution to the address
          below. May satisfy your RMD without increasing taxable income.
        </p>
        <Block label="Recipient legal name">{NONPROFIT.legalName}</Block>
        <Block label="EIN">{NONPROFIT.ein}</Block>
        <Block label="Mailing address">{ADDRESS_LINES.join(" · ")}</Block>
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          Donate first via the calculator above, then submit the receipt to
          your employer&apos;s matching portal (Benevity, Bright Funds,
          YourCause, etc.) using:
        </p>
  ...
  },
  {
    ...
    isReady: () => Boolean(NONPROFIT.brokerage.firmName),
    ...
    body: (
      <>
        <p>
          ...
          market value and avoid capital gains. Have your broker initiate a
          DTC transfer to:
        </p>
        ...
        <Block label="Account name">{NONPROFIT.brokerage.accountName}</Block>
  ...
  },
  {
    ...
    isReady: () => Boolean(NONPROFIT.givingBlockWidgetId),
    who: "Best for crypto with long-term unrealized gains.",
    body: (
      <>
        <p>
          Donate crypto held more than a year direct as property — preserves
          your deduction at fair market value, avoids capital gains. Auto-converts
          to USD on receipt; you receive an IRS Form 8283 receipt.
        </p>
        {/* TODO: embed The Giving Block widget once {NONPROFIT.givingBlockWidgetId} is set. */}
        <p className="text-xs text-neutral-600">
          Powered by The Giving Block (custodial; auto-receipt).
        </p>
      </>
    ),
  },
...
export function WaysToGiveCard() {
  ...
  const visibleMethods = METHODS.filter((m) => (m.isReady ? m.isReady() : true));
      ...
      <details>
        ...
        <div className="mt-4 space-y-4">
          <p className="max-w-3xl text-sm leading-6 text-neutral-700">
            Major-gift routes that can reduce your taxes or processing fees.
            Not tax advice — talk to your CPA. U.S.-specific.{" "}
