...
- **Use before/after screenshot review pages.** For meaningful UI changes, generate a local HTML comparison under `packages/web/output/playwright/` with the previous screenshot shown above the updated screenshot for each page/state/viewport. If true before screenshots are not practical, make an after-only review page and say so.
- **Make screenshot review images full-width by default.** Show review screenshots in a single column at full available width so mobile and desktop captures are actually inspectable. Use side-by-side layouts only when the human explicitly asks for that comparison.
