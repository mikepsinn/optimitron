...
- **Use the stable screenshot review page.** For meaningful UI changes, generate the local HTML comparison at `packages/web/output/playwright/review/latest.html` with the previous screenshot shown above the updated screenshot for each page/state/viewport. If true before screenshots are not practical, make an after-only review page and say so.
