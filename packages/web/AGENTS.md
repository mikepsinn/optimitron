...
- **Validate copy, then ship it.** For requested copy changes, commit and push checked public copy promptly; wait for pre-commit human review only when explicitly requested.
...
- **Screenshot every UI change before committing.** For requested UI changes, capture and inspect the affected surface, then commit and push promptly so CI starts. Report screenshots afterward; wait for pre-commit human review only when explicitly requested.
