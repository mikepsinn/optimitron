 ...
 *
 * THE demo script. One command shows the complete Optimitron pipeline:
 *
 *   pnpm --filter @optimitron/examples run demo:health
 *
...
function generateGoldenPathReport(
  ...
  add('# 🏥 Optimitron — Golden Path Health Optimization Report');
  add('');
  ...
  add('');
  add('This report demonstrates the complete Optimitron causal analysis pipeline.');
  ...
  add('---');
  add('');
  add('*Generated by `@optimitron/examples` golden path demo using `@optimitron/optimizer`.*');
...
function printConsoleSummary(
  ...
  console.log('\n' + '═'.repeat(70));
  console.log('  🏥 OPTIMITRON — Golden Path Health Optimization Demo');
