...
import { ReminderComposer } from "../tasks/reminder-composer";
...
/**
 * Optimitron's session carries `countryCode`; the campaign apps' does not.
 * Reading it structurally keeps one component usable from both without
 * widening every app's NextAuth session type for a field only this composer
 * wants. Absent or non-string means "fall back to the browser locale".
 */
function getSessionCountryCode(user: unknown): string | null {
  if (!user || typeof user !== "object") return null;
  const value = (user as Record<string, unknown>).countryCode;
