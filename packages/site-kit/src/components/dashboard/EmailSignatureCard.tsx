...
export function EmailSignatureCard({ referralLink, userName, frameless = false, className }: EmailSignatureCardProps) {
  const [copied, setCopied] = useState(false)

  // Generate HTML email signature
  const generateSignature = () => {
    const websiteLabel = getShareWebsiteLabel(referralLink)
  ...
  <p style="margin: 6px 0 0 0; font-family: Arial, sans-serif; font-size: 12px; color: #444;">
    It takes only ${SHARE_COPY_FACTS.nuclearWinterWarheads} nuclear weapons to trigger a nuclear winter. Humanity budgets for ${SHARE_COPY_FACTS.apocalypseBudget} apocalypses.
  </p>
  ...
  <p style="margin: 4px 0 0 0; font-family: Arial, sans-serif; font-size: 11px;">
    <a href="${referralLink}" style="color: #FF6B9D; font-weight: bold; text-decoration: none;">Take 30 seconds to eradicate disease at ${websiteLabel}</a>
  </p>
</div>`
  }
