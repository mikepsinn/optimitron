...
} from "./efficacy-lag-matcher.server";
import { findCanonicalConditionGlobalVariable } from "./global-variable-lookup.server";
import { ensureHumanityVGovernmentPlaintiffParty } from "./court-enrollment.server";
import { buildDisplayNameFromParts } from "./person-name";
import {
  isSelfServeMemorialEvidenceKindAllowed,
  shouldPublishRepresentedCondition,
} from "./represented-person-privacy";
import { slugify } from "./slugify";
import { ensureSubjectForPerson } from "./subject.server";
...
export interface RepresentedPersonRegistrationDependencies {
  prisma: PrismaClient;
  requireAuth: () => Promise<{ userId: string }>;
  ensurePersonForUser: (userId: string) => Promise<{ id: string }>;
}

export type RepresentedPersonRouteContext = { params: Promise<{ slug: string }> };

/** Generic treaty representation; Court enrollment uses the shared transaction helper. */
export async function submitTreatyRepresentedPerson(
  request: Request,
  { params }: RepresentedPersonRouteContext,
  { prisma, requireAuth, ensurePersonForUser }: RepresentedPersonRegistrationDependencies,
) {
  try {
