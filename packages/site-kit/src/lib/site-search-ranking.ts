/**
 * Pure relevance scoring for site search.
 *
 * No imports on purpose: the campaign apps run this in a server component and
 * the tests run it in isolation, so it must not drag Prisma, next-auth, or a
 * route registry behind it.
 *
 * Optimitron keeps its own copy at `apps/optimitron/src/lib/site-search-ranking.ts`
 * because that app does not depend on site-kit. The two are byte-identical
 * below this comment; when Optimitron's own /search retires (#244/#247) its
 * copy goes with it rather than being merged back.
 */
