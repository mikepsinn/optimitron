...
// describeGrade and fmt are imported from @optimitron/optimizer
...
export function generatePolicyReport(analysis: PolicyRankingResult): string {
  ...
  if (analysis.overallWelfareScore !== undefined) {
    lines.push(`Overall welfare score: **${fmt(analysis.overallWelfareScore, 1)}**.`);
  }
  ...
  if (policies.length > 0) {
    ...
    for (let i = 0; i < ranked.length; i++) {
      ...
      lines.push(
        ...
        `| ${fmt(ws, 1)} ` +
        `| ${wsCi ? `[${fmt(wsCi[0], 1)}, ${fmt(wsCi[1], 1)}]` : '—'} ` +
    ...
    if (actionable.length === 0 && maintained.length === 0) {
      ...
      if (actionable.length > 0) {
        for (let i = 0; i < actionable.length; i++) {
          ...
          if (welfareCi) {
            lines.push(`   - Welfare 95% CI: ${fmt(welfareCi[0], 1)} to ${fmt(welfareCi[1], 1)}`);
          }
  ...
  lines.push('');
  lines.push(
    `*Generated by \`@optimitron/opg\` on ${reportTimestamp()} · ` +
    `${policies.length} polic${policies.length === 1 ? 'y' : 'ies'} analyzed · ` +
