...
describe('calculatePolicyImpactScore', () => {
  it('returns a valid PolicyImpactScore object', () => {
    const score = calculatePolicyImpactScore(cloneResult());
    expect(PolicyImpactScoreSchema.safeParse(score).success).toBe(true);
  });

  it('uses effect size direction to set score sign', () => {
    const negative = cloneResult();
    negative.effectSize.absoluteChange = -2;
    const score = calculatePolicyImpactScore(negative);
  ...
  it('returns zero when effect direction is neutral', () => {
    ...
    neutral.effectSize.absoluteChange = 0;
    const score = calculatePolicyImpactScore(neutral);
    expect(score.score).toBe(0);
    expect(score.effectDirection).toBe(0);
  });

  it('reflects confidence based on p-value', () => {
    const lowConfidence = cloneResult();
    lowConfidence.pValue = 0.9;
    const score = calculatePolicyImpactScore(lowConfidence);
    expect(score.confidence).toBeCloseTo(0.1, 6);
  });

  it('saturates sample size and supports custom saturation', () => {
    const result = cloneResult();
    result.numberOfPairs = 10;
    const defaultScore = calculatePolicyImpactScore(result);
    const fastScore = calculatePolicyImpactScore(result, { sampleSizeSaturation: 10 });
    expect(fastScore.sampleSizeFactor).toBeGreaterThan(defaultScore.sampleSizeFactor);
  });
});
