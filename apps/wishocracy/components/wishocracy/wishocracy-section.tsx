  ...
  return (
    <section className="bg-brutal-yellow pt-4 pb-32 border-b-4 border-primary min-h-screen">
      <div className="mx-auto max-w-6xl px-4">
        ...
        {/* Status Bar with Sign In Prompt */}
        <WishocracyStatusBar
          show={!state.isLoading && !state.showIntro && !state.showCategorySelection}
          isLoading={state.isLoading}
          isAuthenticated={state.status === "authenticated"}
          referralCode={state.searchParams?.get("ref") ?? null}
        ...
        {/* Edit Section - Only for authenticated users when survey is complete */}
        {state.status === "authenticated" && !state.showIntro && !state.showCategorySelection && state.comparisons.length >= state.totalPossiblePairs && (
