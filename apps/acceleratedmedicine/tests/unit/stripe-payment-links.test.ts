 ...
 * account. In 2025 the checked-in Payment Links were generated against a
 * personal account, so donate buttons on two sites sent donations to the
 * wrong legal entity. These tests fail the build if that ever recurs.
 *
 * Payment Link URLs are opaque slugs, but every Stripe object ID embeds the
 * owning account's suffix (acct_1TPCJF>D5epyB9qRx< appears in
 * plink_/prod_/price_ IDs), and the generator records the IDs from the same
 * API objects the URLs come from.
 */
