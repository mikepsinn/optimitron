...
const Card = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => (
  <div
    ...
    data-slot="card"
    className={cn(
      "text-card-foreground flex flex-col gap-6 rounded-md border-2 border-foreground bg-card py-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]",
      className,
    )}
    {...props}
  />
...
const CardTitle = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => (
  <div ref={ref} data-slot="card-title" className={cn("leading-none font-black uppercase tracking-tight", className)} {...props} />
))
CardTitle.displayName = "CardTitle"

const CardDescription = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(
  ({ className, ...props }, ref) => (
    <div ref={ref} data-slot="card-description" className={cn("text-muted-foreground text-sm font-bold", className)} {...props} />
  ),
)
