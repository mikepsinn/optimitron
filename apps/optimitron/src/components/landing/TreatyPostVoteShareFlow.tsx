...
const majorityHumanityText = formatFlowWords(
  FLOW_MAJORITY_OF_HUMANS_ON_EARTH,
  1,
);
...
function FlowParagraph({
  ...
  return (
    <TreatyFlowParagraph dropCap={dropCap}>{children}</TreatyFlowParagraph>
  );
}
...
function DetailsBlock({
  ...
  return (
    ...
    >
      <summary className="cursor-pointer font-black uppercase tracking-[0.12em] text-[var(--treaty-ink)]">
        {summary}
      </summary>
...
function PromotionScreen({
  ...
  onChoice: (target: "friend" | "humanity") => void;
}) {
  ...
  return (
    <>
      <div className="space-y-5">
        ...
        <FlowParagraph>
          You have been promoted to <strong>Humanity Manager</strong> at{" "}
        ...
        <dl className="space-y-3 border-y border-[var(--treaty-ink)]/30 py-4 text-sm font-bold leading-7 text-[var(--treaty-ink-soft)] sm:text-base">
          <div>
            <dt className="text-xs font-black uppercase tracking-[0.14em] text-[var(--treaty-ink-muted)]">
              Employees
            </dt>
          ...
          <div>
            ...
            <dd>
              ~<ParameterValue param={TREATY_HALE_GAIN_YEAR_15} figures={3} />{" "}
              extra years of healthy life + ~
              <ParameterValue
                param={TREATY_TRAJECTORY_LIFETIME_INCOME_GAIN_PER_CAPITA}
                figures={3}
              />{" "}
              additional lifetime income. Vesting: treaty must pass. Forfeited
              on dismissal.
            </dd>
          </div>
        </dl>
        <FlowParagraph>
          <strong>Performance to date:</strong> ~
          <strong>{deathCount.toLocaleString()}</strong> preventable deaths
          since you started reading. Counter resets when you assign a task.
        </FlowParagraph>
      </div>
      <FlowButtonRow>
        <Button
          className={dismissButtonClass}
          onClick={() => onChoice("humanity")}
        >
          Manage humanity
        </Button>
        <Button
          className={primaryButtonClass}
          onClick={() => onChoice("friend")}
        >
          Manage a friend
        </Button>
      </FlowButtonRow>
...
function milestoneCopy(sentCount: number) {
  const milestoneLives = formatLives(
    sentCount * FLOW_VOTER_LIVES_SAVED_ROUNDED.value,
  );
...
export function TreatyPostVoteShareFlow({
  ...
  const [dismissiveCount, setDismissiveCount] = useState(
  ...
  const [completedInvitationIds, setCompletedInvitationIds] = useState<
    Set<string>
  >(new Set());
  ...
  const go = useCallback(
    (next: FlowScreen, dismissive = false) => {
  ...
  const createInvitation = useCallback(
    async (options: { messageText?: string | null } = {}) => {
  ...
  const completeCurrentInvitation = useCallback(() => {
    ...
    setLastRecipientName(
      getReferralInvitationFirstName(invitation.recipientName),
    );
        ...
        return (
          <>
            <div className="space-y-4">
              ...
              <FlowParagraph>
                Statistically, you and/or someone you love will get a horrible
                disease.{" "}
                <ParameterValue
                ...
                <ParameterValue param={SAFE_COMPOUNDS_COUNT} figures={2} />{" "}
                known-safe compounds sit on shelves, and{" "}
                <ParameterValue param={UNEXPLORED_RATIO} figures={3} /> of their
                potential uses have never been tested — because the money was
                busy turning into missiles.
              </FlowParagraph>
            </div>
            <FlowButtonRow>
              <Button
                className={dismissButtonClass}
                onClick={() => go(nextAfterStakes, true)}
              >
                I have chosen disease
              </Button>
              <Button
                className={primaryButtonClass}
                onClick={() => go(nextAfterStakes)}
              >
                Okay, go on
              </Button>
            </FlowButtonRow>
        ...
        return (
          <>
            <div className="space-y-4">
              {alt ? (
                <>
                  ...
                  <FlowParagraph>
                    You trigger a chain reaction that gets a majority of humans
                    on Earth —{" "}
                    <ParameterValue
                      param={FLOW_MAJORITY_OF_HUMANS_ON_EARTH}
                      figures={1}
                    />{" "}
                    people — to collectively agree: &quot;Yes, we are willing to
                    sacrifice one apocalypse of our{" "}
                    <ParameterValue
                      param={FLOW_NUCLEAR_WINTER_OVERKILL_FACTOR}
                      figures={3}
                    />{" "}
                    apocalypse capacity in exchange for eradicating disease
                ...
                <>
                  <FlowParagraph>
                    Imagine you triggered a chain reaction that got a majority
                    of humans on Earth —{" "}
                    <ParameterValue
                      param={FLOW_MAJORITY_OF_HUMANS_ON_EARTH}
                      figures={1}
            ...
            <FlowButtonRow>
              <Button
                className={dismissButtonClass}
                onClick={() => go("twoHumans", true)}
              >
                Not neat
              </Button>
              <Button
                className={primaryButtonClass}
                onClick={() => go("twoHumans")}
              >
                Neat
              </Button>
            </FlowButtonRow>
            ...
            <div className="space-y-4">
              {alt ? (
                <>
                  ...
                  <FlowParagraph>
                    Tell 2 friends. They tell 2.{" "}
                    {FLOW_DOUBLING_ROUNDS_TO_TARGET} rounds reaches{" "}
                    <ParameterValue
                      param={FLOW_MAJORITY_OF_HUMANS_ON_EARTH}
                      figures={1}
                    />
                    . {FLOW_DOUBLING_MONTHS_AT_WEEKLY_PACE} months at one per
                    week.
                  </FlowParagraph>
                  <FlowParagraph>
                    Yes, this is technically a chain letter. The old ones
                    threatened 7 years of bad luck. If this chain breaks, you
                    and everyone you love will suffer and die of curable
                    diseases. Which is also bad luck.
                  </FlowParagraph>
                </>
              ) : (
                <>
                  <FlowParagraph>
                    Tell 2 friends. They tell 2 friends.{" "}
                    {FLOW_DOUBLING_ROUNDS_TO_TARGET} rounds reaches{" "}
                    <ParameterValue
                      param={FLOW_MAJORITY_OF_HUMANS_ON_EARTH}
                      figures={1}
                    />{" "}
                    humans (a majority of humanity). That&apos;s{" "}
                    {FLOW_DOUBLING_ROUNDS_TO_TARGET} days at one per day,{" "}
                    {FLOW_DOUBLING_MONTHS_AT_WEEKLY_PACE} months at one per
                    week. Everyone else can ignore you.
                  </FlowParagraph>
                  <FlowParagraph>
                    Yes, this is technically a chain letter. The old ones
                    threatened 7 years of bad luck. If this chain breaks, you
                  ...
                  >
                    ...
                    <p>
                      This one requires touching a glowing rectangle a few
                      times. It costs nothing. There are no stamps. And the
                      threat — that you and everyone you love will suffer and
                      die of curable diseases if nobody funds the research — is
                      not a superstition. It is an epidemiological fact.
                    </p>
          ...
          <>
            <div className="space-y-4">
              {alt ? (
                <>
                  <FlowParagraph>
                    Last math. Then you can return to your regularly scheduled
                    apathy.
                  </FlowParagraph>
                  ...
                  <p className="text-center text-xl font-black leading-tight sm:text-left">
                    One vote = {voterLivesSavedText} lives saved.
                  </p>
                  <FlowParagraph>
                    Every friend you get to vote adds another lifetime to your
                    Inverse Kills Score.
                  </FlowParagraph>
                </>
              ) : (
                <>
                  ...
                  >
                    <p>
                      When a majority of humans on Earth publicly agree that
                      letting their families die for{" "}
                      <ParameterValue
                        param={FLOW_NUCLEAR_WINTER_OVERKILL_FACTOR}
                        figures={3}
                      />{" "}
                      apocalypses is idiotic, no politician can refuse the trade
                      without losing their seat.{" "}
                      <ParameterValue
                        param={FLOW_TOTAL_LIVES_SAVED}
                        figures={3}
                      />{" "}
                      deaths prevented ÷ {majorityHumanityText} ={" "}
                      <strong>
                        <ParameterValue
                          param={FLOW_VOTER_LIVES_SAVED}
                          figures={4}
                        />{" "}
                        lives per vote
                      </strong>
                      ...
                      <ParameterValue
                        ...
                        figures={3}
                      />{" "}
                      hours of suffering prevented ÷ {majorityHumanityText} ={" "}
                      <strong>
                        <ParameterValue
                          param={FLOW_VOTER_SUFFERING_HOURS_PREVENTED}
                          figures={4}
                        />{" "}
                        hours per vote
                      </strong>
                      . At <ParameterValue param={HOURS_PER_YEAR} figures={3} />{" "}
                      hours/year ={" "}
                      <strong>
                        ~
                        <ParameterValue
                  ...
                  <FlowParagraph>
                    Your vote already did this. Every friend you get to vote
                    adds another lifetime to your Inverse Kills Score.
                  </FlowParagraph>
                </>
              )}
            </div>
            <FlowButtonRow>
              ...
              <Button
                className={primaryButtonClass}
                onClick={() =>
                  go(answer === "yes" ? "representPerson" : "promotion")
                }
              >
      ...
      case "representPerson":
        return (
          <>
            <div className="space-y-4">
              <FlowParagraph>
                Thank you for voting. Now: would you like to vote on behalf of
                someone who can&apos;t? Someone who died of a disease that might
                have been cured, or a war that didn&apos;t need to happen? Give
                them a voice. They would have voted too.
              </FlowParagraph>
            </div>
      ...
      case "sendMessage":
        return (
          <>
            <div className="space-y-5">
              {alt ? (
                <FlowParagraph>One at a time. Bear with me.</FlowParagraph>
              ) : null}
              <FlowParagraph>
                {sentCount === 0
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label
                    ...
                    htmlFor="post-vote-recipient-name"
                  >
                    To
                  </Label>
                  <Input
                ...
                <div className="space-y-2">
                  <Label
                    className="text-xs font-black uppercase"
                    htmlFor="post-vote-recipient-email"
                  >
                    Email optional
                  </Label>
                  <Input
                ...
                <FlowButtonRow>
                  ...
                  {recipientEmail.trim() ? (
                    <Button
                      ...
                      disabled={isCreating || isSending || trimmedName === ""}
                    >
      ...
      case "copyConfirm":
        return (
          <>
            ...
            <Button
              className={primaryButtonClass}
              onClick={completeCurrentInvitation}
            >
              I sent it
            </Button>
          </>
        ...
        return (
          <>
            {sentCount <= 1 ? (
              ...
              <div className="space-y-4">
                <FlowParagraph>
                  {`${recipientLabel} added to your employees. Pending: `}
                  <strong>{sentCount}</strong>
                  {` lifetimes / `}
                  <strong>{pendingLives}</strong>
                  {` lives.`}
                </FlowParagraph>
        ...
        return (
          <>
            <div className="space-y-4">
              ...
              <FlowParagraph>
                Then you get to go back to whatever you were doing before the
                most important thing in the universe rudely interrupted.
              </FlowParagraph>
            </div>
            <Button
              className={primaryButtonClass}
            ...
            <div className="space-y-4">
              {sentCount === 0 ? (
                <>
                  ...
                  <FlowParagraph>
                    What would we have to change to make you send it to everyone
                    you love?
                  </FlowParagraph>
                </>
              ) : sentCount >= 5 ? (
                ...
                <>
                  ...
                  <FlowParagraph>
                    What would we have to change about this to make you send it
