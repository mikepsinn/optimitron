...
describe("MCP server tool dispatch", () => {
      ...
      mocks.getTaskCoordinationContext.mockResolvedValue({
        activeExecution: {
          ...
          runContext: {
            ...
            ownedFileGlobs: ["apps/optimitron/src/lib/**"],
  ...
  describe("repository and site inventory tools", () => {
      ...
      const fetchMock = vi.fn(async (url: string) => {
        ...
        return jsonResponse({
          ...
          items: [
            {
              name: "mcp-server.ts",
              path: "apps/optimitron/src/lib/mcp-server.ts",
              sha: "abc123",
              html_url:
                "https://github.com/mikepsinn/optimitron/blob/main/apps/optimitron/src/lib/mcp-server.ts",
    ...
    it("getFileContent and listRepoFiles read through GitHub Contents API", async () => {
      ...
      const fetchMock = vi.fn(async (url: string) => {
        if (url.includes("apps%2Foptimitron%2Fsrc%2Fvalue.ts")) {
          return jsonResponse(
            {
              content: Buffer.from(source, "utf8").toString("base64"),
              encoding: "base64",
              html_url:
                "https://github.com/mikepsinn/optimitron/blob/main/apps/optimitron/src/value.ts",
              name: "value.ts",
              path: "apps/optimitron/src/value.ts",
              sha: "sha-file",
              size: source.length,
              type: "file",
            },
        ...
        return jsonResponse([
          {
            html_url:
              "https://github.com/mikepsinn/optimitron/tree/main/apps/optimitron/src",
            name: "lib",
            path: "apps/optimitron/src/lib",
            sha: "sha-dir",
            size: 0,
            type: "dir",
          },
      ...
      const listResult = await client.callTool({
        name: "listRepoFiles",
        arguments: { repo: "optimitron", path: "apps/optimitron/src" },
      });
      ...
      expect(parseToolBody(listResult).entries).toEqual([
        expect.objectContaining({ path: "apps/optimitron/src/lib", type: "dir" }),
      ]);
    });
  ...
  describe("tracking reminders and measurements", () => {
    ...
    it("returns yesterday and older stored overdue occurrences without hiding the generation window", async () => {
      ...
      try {
        ...
        const old = {
          ...
          status: "SENT",
          trackingReminder: EXISTING_TRACKING_REMINDER,
          trackedValue: null,
        };
        mocks.trackingReminderNotificationFindMany.mockImplementation(
          async (args: {
            where: { notifyAt: { gte?: Date; lt: Date }; userId: string };
            include?: unknown;
          }) => {
            // Emulate the persisted boundary: the old row is excluded by a date lower bound.
            expect(args.where.userId).toBe("user-1");
            return args.include &&
              (!args.where.notifyAt.gte ||
                args.where.notifyAt.gte <= old.notifyAt)
              ? [old]
              : [];
          },
        );
        mocks.measurementFindMany.mockResolvedValue([
          { globalVariableId: "gv-vitd", startTime: old.notifyAt },
        ]);
        const client = await setup("user-1", ALL_SCOPES);
        const result = await client.callTool({
          name: "listTrackingReminderNotifications",
        ...
        expect(result.isError).toBeFalsy();
        const body = parseToolBody(result) as {
          notifications: Array<{ dateKey: string; due: string }>;
          backlog: unknown;
        };
        expect(body.backlog).toMatchObject({
          generatedStartDateKey: "2026-08-07",
          generatedEndDateKey: "2026-08-20",
          storedNotifications: "all outstanding dates",
        });
        expect(body.notifications).toHaveLength(15);
        expect(body.notifications[0]).toMatchObject({
          dateKey: "2026-08-20",
          due: "2026-08-20T08:00:00-05:00",
        });
        expect(body.notifications[1]).toMatchObject({
          dateKey: "2026-08-19",
          status: "OVERDUE",
        });
        expect(body.notifications.at(-1)).toMatchObject({
          dateKey: "2026-07-02",
          globalVariableId: "gv-vitd",
          nOf1VariableId: "nof1-1",
          sameDayMeasurementCount: 1,
        });
        const explicit = await client.callTool({
          name: "listTrackingReminderNotifications",
          arguments: { status: "OVERDUE", dateKey: "2026-08-19" },
        });
        expect(parseToolBody(explicit)).toMatchObject({
          dateKey: "2026-08-19",
          notifications: [expect.objectContaining({ dateKey: "2026-08-19" })],
        });
        expect(parseToolBody(explicit)).not.toHaveProperty("backlog");
      } finally {
        vi.useRealTimers();
      }
    });
