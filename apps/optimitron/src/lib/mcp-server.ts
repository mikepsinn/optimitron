...
// ---------------------------------------------------------------------------
// Mirrors `TaskContextJsonSchema` in apps/optimitron/src/lib/tasks/task-context.ts
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: "string",
          description:
            "Optional directory path qualifier, e.g. apps/optimitron/src/lib.",
        },
        fileType: {
          type: "string",
...
export function createMcpServer(
  ...
  server.setRequestHandler(
    ...
    async (request: { params: { arguments?: unknown; name: string } }) => {
      ...
      if (!TASK_TOOL_DEFINITIONS.some((tool) => tool.name === name)) {
      ...
      try {
        if (isTrackingToolName(name)) {
          // Extracted family; returns the same ok()/authRequired() envelopes
          // the inline cases produced, so no normalization pass is needed.
          return await handleTrackingToolCall({
            args: a,
            authRequired,
            name,
            userId: userId ?? null,
        ...
        switch (name) {
          ...
          case "listReferendums": {
            ...
            const where: Prisma.ReferendumWhereInput = {
              deletedAt: null,
              kind: { not: ReferendumKind.COURT_CASE },
              status,
              ...(query
                ? {
                    OR: [
          ...
          case "createReferendum": {
            ...
            if (!kind) {
              return err(
                "kind must be one of GENERAL, DECLARATION, TREATY, MEMBERSHIP, AMENDMENT, or BUDGET",
              );
            }
            if (kind === ReferendumKind.COURT_CASE) {
              return err("Court jury referendums must be created on https://courtofhumanity.org/api/mcp.");
            }
