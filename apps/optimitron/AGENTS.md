...
- **Write from the page and evidence.** Read the rendered page, current source, and user-provided references. State the outcome, mechanism, proof, and action. Preserve citations and legal distinctions.
- **Publish verified copy.** Verify copy and screenshots. Commit and push the intended changes. Open or update a normal pull request. Its review packet is Mike's approval surface; leave its checkboxes unchecked. Pause only for his hold or an unresolved strategic choice.
...
- **Screenshot every UI change before committing.** Capture and inspect the affected surface, then report the screenshots and findings in the handoff. Commit and push without waiting unless the human requests a hold.
...
- **Use the stable screenshot review page.** For meaningful UI changes, generate the local HTML comparison at `apps/optimitron/output/playwright/review/latest.html` with before/after screenshots side by side when both versions are available. The review page should still capture all covered routes, but changed routes or missing-baseline routes should be expanded by default and unchanged routes collapsed. If true before screenshots are not practical, make an after-only review page and say so.
- **Cover every user-facing change.** Every changed route or state must appear in `review/latest.html` as a before/after screenshot or exact copy diff. If absent, fix its review flags, `covers` mapping, or fixture.
- **Run an author preflight.** Inspect every changed or copy-only route in `latest.html`. Fix and rerun until every diff comes from the PR. Report the noise removed.
- **Stabilize noise at its source.** Use deterministic fixtures, `freezeClock`, `data-volatile`, and hydration, animation, and image readiness. Never mask meaningful content, accept a bad baseline, or widen thresholds.
...
- **Create named review folders only when useful.** Do not create duplicate `review.html` files out of habit. A named folder under `apps/optimitron/output/playwright/` is useful for a longer audit, multiple competing versions, or preserving before/after history; otherwise `review/latest.html` is enough.
...
- **Run ad-hoc Playwright scripts from the web package.** Use `pnpm --dir apps/optimitron exec node ...` or run from `apps/optimitron` with `pnpm exec` so `@playwright/test` resolves from the web app's dev dependencies.
