 ...
 * Run: cd apps/optimitron && pnpm exec node --test scripts/visual-review-diff.test.mjs
 */
...
test("copy diffs ignore recreated preview record ids", () => {
  ...
  assert.equal(
    normalizeVisualReviewMarkdown(
      before,
      "apps/optimitron/src/app/tasks/page.logged-in.md",
    ),
    normalizeVisualReviewMarkdown(
      after,
      "apps/optimitron/src/app/tasks/page.logged-in.md",
...
test("calendar copy diffs ignore the capture date and generated schedule time", () => {
  ...
  const snapshotPath = "apps/optimitron/src/app/calendar/page.logged-in.md";
