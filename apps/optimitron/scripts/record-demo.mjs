 ...
 *
 * Usage: node apps/optimitron/scripts/record-demo.mjs [playlist]
 * Default playlist: hackathon
 *
 * Navigates through each slide, waiting the estimated narration duration.
 * Outputs a .webm video file to apps/optimitron/public/demo-videos/
 */
...
async function main() {
  ...
  const context = await browser.newContext({
    ...
    recordVideo: {
      dir: 'apps/optimitron/public/demo-videos/',
  ...
  for (let i = 0; i < totalSlides; i++) {
    ...
    await page.screenshot({
      path: `apps/optimitron/public/demo-videos/${PLAYLIST}-slide-${String(i + 1).padStart(2, '0')}.png`,
      type: 'png',
    });
  ...
  console.log(`\nDone! Video saved to apps/optimitron/public/demo-videos/`);
