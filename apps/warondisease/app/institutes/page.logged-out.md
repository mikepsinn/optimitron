# /institutes

## Metadata

- Page title: The International Campaign to End War and Disease
- Meta description: Click a glowing rectangle. 15 seconds. 2.6 lives saved + 53 years of suffering prevented.
- Canonical: https://warondisease.org/institutes
- Open Graph title: The International Campaign to End War and Disease
- Open Graph description: Click a glowing rectangle. 15 seconds. 2.6 lives saved + 53 years of suffering prevented.
- Open Graph image: https://warondisease.org/assets/warondisease/war-on-disease-og-1200x630.png
- Twitter title: The International Campaign to End War and Disease
- Twitter description: Click a glowing rectangle. 15 seconds. 2.6 lives saved + 53 years of suffering prevented.

## Visible Page Copy

- [THE INTERNATIONAL CAMPAIGN TO END WAR AND DISEASE](/)
- [Go to Dashboard](/dashboard)
## BECOME A PARTNER INSTITUTE
## FOR THE GLOBAL CLINICAL TRIAL
## ABUNDANCE SURVEY
- Host a short, nonpartisan survey for your supporters, get policy-grade data on how much they want governments to invest in clinical trials, and use it to strengthen your grants, advocacy, and fundraising.
- BECOME A PARTNER NOW
- See how it works ↓
### WHAT THE SURVEY ASKS
- A 2-MINUTE EDUCATIONAL SURVEY ON PUBLIC HEALTH FUNDING PRIORITIES
#### 1. PRIORITY PREFERENCES
- Interactive slider: How would you allocate resources between current budget categories?
#### 2. EDUCATIONAL CONTEXT
- Shows actual data: Current spending ratio between defense and medical research funding globally
#### 3. OPINION QUESTION
- "Would you support international cooperation to increase clinical trial funding?"
- YOUR DASHBOARD: COMMUNITY RESPONSES + PREFERRED RESOURCE ALLOCATION DATA
### THE BRUTAL TRUTH
- [$2.72T](https://manual.WarOnDisease.org/knowledge/economics/1-pct-treaty-impact.html)
- [$5B](https://manual.WarOnDisease.org/knowledge/economics/1-pct-treaty-impact.html)
- [604:1](https://manual.WarOnDisease.org/knowledge/solution/1-percent-treaty.html)
### WHAT 1% CHANGES
- REDIRECTING 1% OF MILITARY BUDGETS = $27.2B/YEAR
- Ready to demonstrate your community's support for clinical trial abundance?
### THE DATA ASSET YOU GET
- EVIDENCE YOU CAN USE IN GRANTS, POLICY BRIEFS, AND DONOR REPORTS
#### GRANT APPLICATIONS
- "72% of our community wants public clinical trial funding at least doubled"
- → Drop into grant proposals to demonstrate community support
#### POLICY TESTIMONY
- "Our 10,000 verified respondents support redirecting 1% of military budgets to trials"
- → Strengthen advocacy with hard numbers, not anecdotes
#### DONOR REPORTS
- "Show supporters: 'Your voice joined [4B](https://manual.WarOnDisease.org/knowledge/strategy/global-referendum.html) humans demanding trial abundance'"
- → Prove the gap between "I care about this" and "I did something about it" is 2 minutes
- YOUR DASHBOARD: EXPORTABLE CHARTS, SEGMENTED BY COUNTRY + DEMOGRAPHICS
### ADD THE SURVEY TO YOUR SITE
- ONCE APPROVED, YOU'LL GET A CUSTOM SURVEY PAGE TO EMBED ON YOUR SITE OR SHARE WITH YOUR SUPPORTERS. THIS SECTION SHOWS THE BEST DEFAULTS.
#### BEST DEFAULT
- Put a direct survey button in emails, social posts, and newsletter flows. Add the iframe on a homepage, "Get Involved" page, or campaign landing page where supporters already expect to act.
#### EMBED IF...
- You want supporters to answer without leaving your site, or you already have a campaign page where a full survey block makes sense.
#### LINK IF...
- Your CMS fights iframes, your web team is cautious, or you mainly want a clean CTA in email, social, or a donate/take-action flow.
- EMBED CODE
- DIRECT LINK
#### IFRAME EMBED CODE
- COPY AND PASTE THIS INTO YOUR WEBSITE HTML:
- COPY EMBED CODE
- TIP: REPLACE "YOUR-ORG-SLUG" WITH YOUR CUSTOM SLUG. YOU'LL GET THAT AFTER APPROVAL.
#### LIVE DEMO EMBED
- This is what the embedded survey looks like inside a normal page container.
- [OPEN DEMO IN NEW TAB](/survey/demo)
#### PLATFORM NOTES
##### WORDPRESS
- Use a Custom HTML block for the iframe. If your setup strips iframe code or your team prefers a safer path, use the hosted link or a normal button block instead.
##### EMAIL & SOCIAL
- Use the direct link in newsletters, social posts, petitions, donate confirmations, and any flow where asking for a full on-site embed would be overkill.
##### SCRIPT TAGS / FABS
- We do not recommend a JavaScript snippet or floating action button by default. Iframes and direct links are easier to approve, easier to debug, and less likely to fight your styles, consent tools, or IT rules.
### COALITION STATUS
- OPT IN TO SHARED OUTPUTS AND PUBLIC RECOGNITION
#### PUBLIC RECOGNITION
- Featured on the coalition partners page with your logo and mission
#### JOINT REPORTS
- Inclusion in briefings and reports using the combined global dataset
#### LEADERBOARDS
- Optional visibility as a top organization by verified survey responses
- POSITION YOUR ORGANIZATION AS A DATA-DRIVEN VOICE ON TRIAL FUNDING AND PATIENT PRIORITIES
- Get your survey links and start collecting responses today
### FUTURE GRANTS
- ELIGIBILITY FOR PERFORMANCE-BASED FUNDING
- PLANNED
#### CLINICAL TRIAL ABUNDANCE INITIATIVE FUND
- A dedicated fund being designed to provide performance-based grants to organizations that:
##### GENERATE RESPONSES
- Collect high-quality survey responses from your community
##### USE THE DATA
- Advance clinical trial access and funding with the results
- DETAILS AND TIMING DEPEND ON DISCUSSIONS WITH MAJOR PHILANTHROPIC AND INSTITUTIONAL PARTNERS.
- EARLY PARTNER INSTITUTES WILL HELP SHAPE THE FUND'S DESIGN.
### HOW IT WORKS
- THREE SIMPLE STEPS TO START COLLECTING DATA
#### CREATE YOUR PARTNER INSTITUTE
- Fill in your organization name below to get started instantly.
- You'll receive onboarding materials and, if useful, an invitation for a short call.
#### ADD THE SURVEY TO YOUR SUPPORTER JOURNEY
- Choose one or both integration options:
- EMBED OPTION
- Paste a small code snippet into a campaign page, "Get involved" page, or a dedicated survey page. The widget loads using your colors and logo.
- HOSTED LINK OPTION
- Add a button or text link: "Take the Global Clinical Trial Abundance Survey." Opens a co-branded survey page.
- Both options include optional email verification (magic link) for one-response-per-person + anti-bot measures.
- Once you are live, you can:
- Log in to see and export results from your traffic.
- Request custom cuts or visualizations if needed.
- Use the findings wherever evidence of supporter priorities for clinical trials strengthens your case.
### SAFE FOR 501(C)(3) ORGANIZATIONS
- NONPARTISAN, EDUCATIONAL, AND COMPLIANT BY DESIGN
#### NONPARTISAN
- ✓No candidate endorsements
- ✓No party affiliations
- ✓No PAC or campaign contributions
#### EDUCATIONAL
- ✓Descriptive data only
- ✓You choose how to use findings
- ✓Supports your advocacy
#### NO COMPETITION
- ✓We don't fundraise from your traffic
- ✓Donate button goes to YOU
- ✓Join list button goes to YOU
- LEGAL FAQ MEMO AVAILABLE FOR YOUR COUNSEL AND BOARD
### WHO CAN JOIN
- THIS PROGRAM IS DESIGNED FOR ORGANIZATIONS THAT CARE ABOUT ACCELERATING CURES
#### DISEASE FOUNDATIONS
- Alzheimer's, cancer, rare diseases, and other condition-specific organizations
#### PATIENT ADVOCACY
- Patient advocacy organizations and caregiver networks
#### TRIAL NONPROFITS
- Nonprofits that run or support clinical trials or trial access
#### RESEARCH CHARITIES
- Organizations focused on prevention, cures, and medical innovation
#### TRIAL NETWORKS
- Trial networks or consortia seeking better evidence of patient priorities
- IF YOU WORK TO REDUCE THE BURDEN OF ANY DISEASE AND ENGAGE PATIENTS, CAREGIVERS, CLINICIANS, OR SUPPORTERS — YOU ARE A GOOD CANDIDATE.
- Does this sound like your organization? Endorse the treaty.
### WHAT "PARTNER INSTITUTE" MEANS
- A SIMPLE DESIGNATION FOR ORGANIZATIONS JOINING THE COALITION
#### A PARTNER INSTITUTE IS:
- Any nonprofit, foundation, or research group that:
- Hosts or promotes the Global Clinical Trial Abundance Survey to its community
- Uses the resulting data to advocate for better clinical trial funding and access
#### NO SEPARATE ENTITY
- There is no separate legal entity to join. You remain fully independent.
#### NO MEMBERSHIP FEE
- Free to join. No hidden costs. No ongoing financial commitment.
- THE DESIGNATION SIMPLY REFLECTS PARTICIPATION IN THE SURVEY COALITION AND ACCESS TO ITS TOOLS AND OUTPUTS
- Name your organization and get your survey links instantly.
- 🛠️ DEV LOGIN (Test User)
- Continue with Google
- Continue with Email
- MAKING SUFFERING OPTIONAL
#### DO SOMETHING
- [ANSWER THE QUESTION](/#vote)
- [REGISTER A PLAINTIFF](https://courtofhumanity.org/plaintiffs)
- [PRESIDENT MANAGEMENT SYSTEM](/employees)
- [FEEDBACK](/feedback)
#### TELL SOMEONE ELSE
- [TELL TWO PEOPLE](/send)
- [GET THE SHIRT](/shirt)
- [HANG UP FLYERS](/poster)
- [GO DOOR TO DOOR](/door-to-door)
- [TREATY SIGNATORIES](/signatories)
#### LEARN SOMETHING
- [HUMANITY V. GOVERNMENT](https://courtofhumanity.org/humanity-v-government)
- [IMPACT ANALYSIS](https://impact.warondisease.org)
- [HOW TO END WAR AND DISEASE](https://manual.warondisease.org)
- [THE PLAN](/the-plan)
- [FAQ](/faq)
- [ABOUT](/about)
#### YOUR ORGANIZATION
- [JOIN AS AN ORGANIZATION](/join)
- [INSTITUTES](/institutes)
- [VOLUNTEER](/contact)
#### CONTACT
- [hello@warondisease.org](mailto:hello@warondisease.org)
- [PRIVACY POLICY](/privacy)
- [TERMS OF SERVICE](/terms)
