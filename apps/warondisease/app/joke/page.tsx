...
// Derived here rather than frozen into a string: Optimitron computed the same
// figures in routes.ts for the nav item, and warondisease declares metadata
// inline, so the parameters stay the source.
...
async function getPersonalReferralUrl() {
  ...
  const referralIdentifier = getHandleOrReferralCode(sessionUser);

  if (!sessionUser?.id || !referralIdentifier) {
    return null;
  }
