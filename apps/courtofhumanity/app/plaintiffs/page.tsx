...
import { PersonFaceTile } from "./person-face-tile"
import { RegisterPlaintiffForm } from "./register-plaintiff-form"
    ...
    <Layout>
      <section className="mx-auto max-w-7xl space-y-10 px-4 py-10 sm:py-14">
        ...
        <section id="register-plaintiff" aria-label="Register a plaintiff">
          <RegisterPlaintiffForm referendumSlug={referendumSlug} />
        </section>
        ...
        <section className="space-y-5 border-t-2 border-foreground pt-8">
          ...
          >
            {people.length > 0 ? (
              people.map((person) => (
                <PersonFaceTile key={person.partyId} person={person} />
              ))
            ) : (
              <article className="col-span-3 border-2 border-foreground bg-background p-8 text-foreground sm:col-span-4 md:col-span-6 lg:col-span-7 xl:col-span-8">
                <p className="text-xs font-black uppercase text-muted-foreground">
          ...
          {totalPages > 1 ? (
            <nav className="flex items-center justify-between gap-3 border-t-2 border-foreground pt-4 text-xs font-black uppercase">
              {currentPage > 1 ? (
                <Link
                  className="border-2 border-foreground bg-background px-4 py-2 text-foreground"
