...
import { SquarePhotoCropper } from "./square-photo-cropper";
...
interface ManagePlaintiffsClientProps {
  currentPage: number;
...
function PersonThumb({ person }: { person: EditableRepresentedPerson }) {
  return (
    <div className="flex h-12 w-12 shrink-0 items-center justify-center border-2 border-foreground bg-background text-sm font-black uppercase">
      {person.imageUrl ? (
        <img
          alt=""
...
export function ManagePlaintiffsClient({
  currentPage,
  initialEditingId = null,
  people,
  referendumSlug,
}: ManagePlaintiffsClientProps) {
  ...
  if (rows.length === 0) {
    return (
      <section className="border-2 border-foreground bg-card p-6 text-card-foreground">
  ...
  return (
    <section className="space-y-4">
      <div className="space-y-3 md:hidden">
        {rows.map((person) => (
          <article
            className="border-2 border-foreground bg-card p-4 text-card-foreground"
            key={person.id}
          >
      ...
      <div className="hidden overflow-hidden border-2 border-foreground bg-card text-card-foreground md:block">
      ...
      >
        <ReactDialog.Portal>
          ...
          <ReactDialog.Content className="fixed left-1/2 top-1/2 z-[101] max-h-[calc(100vh-2rem)] w-[calc(100vw-2rem)] max-w-6xl -translate-x-1/2 -translate-y-1/2 overflow-auto border-2 border-foreground bg-background p-5 text-foreground shadow-none sm:p-6">
            {editingPerson ? (
              <>
                ...
                <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
                  <div className="space-y-3">
                    <button
                      ...
                      className="group relative aspect-square w-full cursor-pointer overflow-hidden border-2 border-foreground bg-background text-foreground"
                  ...
                  <div className="space-y-4">
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <select
                          className="min-h-12 w-full border-2 border-foreground bg-background px-3 font-bold text-foreground"
                          onChange={(event) => {
                    ...
                    {editingPerson.lifeStatus === PersonLifeStatus.DECEASED ? (
                      <div className="grid gap-3 border-2 border-foreground bg-background p-4 sm:grid-cols-2">
                        <div className="space-y-2">
                          <Label className="text-xs font-black uppercase">
                            Date of death
                          </Label>
                        ...
                        <div className="space-y-3 sm:col-span-2">
                          <label className="inline-flex min-h-11 cursor-pointer items-center border-2 border-foreground bg-background px-3 text-xs font-black uppercase tracking-[0.14em]">
                            <Upload
                          ...
                          {editingPerson.evidence.length > 0 ? (
                            <ul className="space-y-2">
                              {editingPerson.evidence.map((evidence, index) => (
                                <li
                                  className="space-y-2 border-2 border-foreground p-3"
                                  key={evidence.id ?? evidence.sourceUrl}
                                >
                                  <select
                                    className="min-h-10 w-full border-2 border-foreground bg-background px-3 text-sm font-bold text-foreground"
                                    onChange={(event) =>
      ...
      >
        <ReactDialog.Portal>
          ...
          <ReactDialog.Content
            className="fixed left-1/2 top-1/2 z-[131] w-[calc(100vw-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 border-2 border-foreground bg-background p-5 text-foreground shadow-none sm:p-6"
            role="alertdialog"
          >
            <ReactDialog.Title asChild>
