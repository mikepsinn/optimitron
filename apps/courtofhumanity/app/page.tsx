...
import { buildHumanityVGovernmentStructuredData } from "@/lib/campaign-structured-data";
...
import { DamagesSensitivityCalculator } from "./humanity-v-government/DamagesSensitivityCalculator";
import { HumanityVGovernmentVerdictVote } from "./humanity-v-government/HumanityVGovernmentVerdictVote";
import { HUMANITY_V_GOVERNMENT_METADATA } from "./humanity-v-government/page-metadata";
...
export default async function HumanityVGovernmentPage() {
  ...
  const [plaintiffCount, verdictStats] = await Promise.all([
    getHumanityVGovernmentPlaintiffCount(),
    getHumanityVGovernmentVerdictStats(userId),
  ]);
  const fullDamagesText = fmtParamValueOnly(
    CORPORATE_DAMAGES_TREBLE_EXPOSURE_PER_CAPITA,
    3,
  );
  ...
  return (
    <Layout>
      <main className="mx-auto max-w-4xl px-4 py-14 sm:px-6">
        <JsonLdScript data={buildHumanityVGovernmentStructuredData()} />
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link className={defaultButtonClassName} href="#verdict">
            Vote on liability
          </Link>
          <Link
            className={defaultButtonClassName}
            href={`${ROUTES.plaintiffs}#register-plaintiff`}
          >
