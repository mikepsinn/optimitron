...
## Background
Many widely-held policy beliefs have weak or contradictory empirical support. Optomitron's causal inference engine can test these claims using real historical data, generating compelling evidence for the website and grant applications.
...
1. Import dataset + `runFullAnalysis` from `@optomitron/optimizer`
...
- All datasets compile with `pnpm --filter @optomitron/data run build`
