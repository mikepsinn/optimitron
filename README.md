...
|----------|----------------------------:|--------|
| Status quo (2.5% growth) | $20,500 | [GDP trajectories](https://manual.warondisease.org/knowledge/economics/gdp-trajectories.html) |
...
1. **Vote** — Sign the [1% Treaty](https://warondisease.org) (takes 30 seconds)
2. **Recruit** — Get two other humans to do the same (target: 4 billion — all voters on Earth)
3. **Allocate** — Decide how the global budget should be split via pairwise comparisons (Wishocracy)
...
A dominant assurance contract that makes voter recruitment individually rational. The mechanism is designed so that participation is positive-expected-value regardless of whether the treaty succeeds — see the [Incentive Alignment Bonds paper](https://iab.warondisease.org) for the formal proof.
...
|------|-------|-------------|
| Earth Optimization Prize | `/prize` | Dominant assurance contract + voter recruitment |
...
| dCongress - Referendums | `/agencies/dcongress/referendums` | Direct democracy voting with proof-of-personhood verification |
